      ...
      <div class="header">
        ...
        <div class="actions">
          <button class="action" @click="importDir" title="Import">
            <i class="fas fa-download" v-if="!importLoading"></i>
      ...
      <div class="explorer-view">
        <ul>
          <li v-for="dir of directories" :key="dir.name" @click="ls(dir.absolutePath)">
            <i class="fas fa-arrow-left" v-if="dir.name === '..'"></i>
            <i class="fas fa-folder" v-else-if="!dir.npmInfos"></i>
            <i class="fab fa-npm" v-else></i>
            {{dir.name}}
          </li>
        </ul>
        <ul>
...
export default {
  ...
  computed: {
    ...
    files() { return this.dir.filter(entry => !entry.isDirectory)}
  },
  data() {
    return {
      homeDir: '',
